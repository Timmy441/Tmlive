...
function showSettings() {
    const currentBase = getDeploymentApiBase();
    const nextBase = prompt('Backend API URL for this deployment:', currentBase);
    if (nextBase !== null) {
        const result = setDeploymentBase(nextBase);
        addNotification(result.apiBase ? `Backend URL saved: ${result.apiBase}` : 'Backend URL cleared', 'success');
    }
    const settings = {
        darkMode: isDarkMode,
        notificationsEnabled: Notification.permission === 'granted',
        soundEnabled: true,
        apiBase: getDeploymentApiBase() || 'same-origin',
        socketUrl: getDeploymentSocketUrl()
    };
...
const defaultSupportQuickReplies = [
    'How do I sign up?',
    'How do I go live?',
    'How do payouts work?',
...
function renderSupportQuickReplies(replies = []) {
    const container = document.getElementById('supportQuickReplies');
    if (!container) return;
    const items = (replies.length ? replies : defaultSupportQuickReplies).slice(0, 4);
    container.innerHTML = items.map(reply => `
        <button class="support-quick-reply" onclick="sendSupportQuickReply('${escapeHtml(reply).replace(/'/g, "\\'")}')">${escapeHtml(reply)}</button>
    `).join('');
}

window.sendSupportQuickReply = function(text) {
...
function renderSupportMessages() {
    ...
    container.innerHTML = supportMessages.map(msg => `
        <div class="support-message ${msg.sender}">
            ${escapeHtml(msg.text)}
            ${msg.meta?.category ? `<div class="support-message-meta">${escapeHtml(msg.meta.category)}</div>` : ''}
        </div>
    `).join('');
    container.scrollTop = container.scrollHeight;
}

function addSupportMessage(text, sender = 'bot', meta = {}) {
    supportMessages.push({ text, sender, time: getCurrentTime(), meta });
...
async function sendSupportMessage() {
    ...
    try {
        const res = await fetch(deploymentApiUrl('/api/chatbot'), {
            method: 'POST',
        ...
        if (!res.ok) {
            ...
            renderSupportQuickReplies(defaultSupportQuickReplies);
            return;
        }
        addSupportMessage(data.answer || 'I am here 24/7 and ready to help.', 'bot', { category: data.category || 'general' });
        renderSupportQuickReplies(data.quickReplies || defaultSupportQuickReplies);
    } catch (err) {
...
function initSupportWidget() {
    ...
    if (closeBtn) closeBtn.addEventListener('click', closeSupportPanel);
    if (toggleBtn) toggleBtn.addEventListener('click', toggleSupportPanel);
    renderSupportQuickReplies(defaultSupportQuickReplies);
}
...
async function followUser(username, btn) {
    try {
        const res = await fetch(deploymentApiUrl(`/api/auth/follow/${username}`), {
            method: 'POST',
...
async function unfollowUser(username, btn) {
    try {
        const res = await fetch(deploymentApiUrl(`/api/auth/unfollow/${username}`), {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${tmToken}` }
        });
        const data = await res.json();
...
async function checkFollowStatus(username, btn) {
    try {
        const res = await fetch(deploymentApiUrl(`/api/auth/profile/${username}`), {
            headers: { 'Authorization': `Bearer ${tmToken}` }
        });
        const data = await res.json();
        if (res.ok) {
...
async function confirmGift() {
    ...
    try {
        const res = await fetch(deploymentApiUrl('/api/auth/send-gift'), {
            method: 'POST',
            headers: {
